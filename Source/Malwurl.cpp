...
DEFINE_APPLICATION_MAIN(Malwurl)

struct MATERIALS {
   static const String array[30];
};

Malwurl::Malwurl(Context* context) :
    Sample(context),
    yaw_(0.0f),
    pitch_(0.0f),
    animate_(false),
...
void Malwurl::CreateInstructions()
{
    ResourceCache* cache = GetSubsystem<ResourceCache>();
    ...
    instructionText->SetText(
        "Use ZQSD keys and mouse to move\n"
...
void Malwurl::SubscribeToEvents()
{
    // Subscribe handler; invoked whenever a mouse click event is dispatched
    SubscribeToEvent(E_UIMOUSECLICK, HANDLER(Malwurl, HandleControlClicked));
}

void Malwurl::SubscribeToEventsVisu()
{
    // Subscribe HandleUpdate() function for processing update events
    SubscribeToEvent(E_UPDATE, HANDLER(Malwurl, HandleUpdate));

    // Subscribe HandlePostRenderUpdate() function for processing the post-render update event, during which we request
    // debug geometry
    SubscribeToEvent(E_POSTRENDERUPDATE, HANDLER(Malwurl, HandlePostRenderUpdate));
}

void Malwurl::MoveCamera(float timeStep)
{
    ...
    // Read ZQSD (French layout for WASD) keys and move the camera scene node to the corresponding direction if they are pressed
    // Acceleration
...
bool Malwurl::Raycast(float maxDistance, Vector3& hitPos, Node*& hitNode)
{
    hitNode = 0;
...
void Malwurl::AnimateObjects(float timeStep)
{
...
void Malwurl::HandleUpdate(StringHash eventType, VariantMap& eventData)
{
...
void Malwurl::HandlePostRenderUpdate(StringHash eventType, VariantMap& eventData)
{
    SetPathPoint();
...
void Malwurl::HandleControlClicked(StringHash eventType, VariantMap& eventData)
{
    String name;
